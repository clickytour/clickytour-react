...
async function getGdprHtml() {
  const response = await fetch('https://clickytour.com/wp-json/wp/v2/pages?slug=gdpr-compliance', {
    headers: {
  ...
  if (!response.ok) {
    throw new Error('Failed to fetch GDPR Compliance content');
  }
    ...
    <PageShell>
      <section className="bg-gradient-to-r from-[#0F2B46] to-[#164E73] text-white py-14 md:py-16">
        <div className="container">
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold mt-2">GDPR Compliance</h1>
        </div>
      </section>
          ...
          <article className="card p-6 md:p-10 max-w-4xl mx-auto">
            <div
              ...
              dangerouslySetInnerHTML={{
                __html:
                  html ||
                  '<p>GDPR Compliance content is currently unavailable. Please check back soon.</p>',
              }}
            />
          </article>
        </div>
