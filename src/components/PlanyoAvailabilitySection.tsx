...
export function PlanyoAvailabilitySection({
  ...
  const selectedPropertyOption = useMemo(() => {
    ...
    const candidates = candidateNights
      .map((targetNights) => {
        const opt = getNearestOptions(targetNights, 1)[0];
        if (!opt) return null;
        return { ...opt, targetNights, nightDelta: Math.abs(targetNights - requestedRangeNights) };
      })
      .filter((v): v is SuggestedOption & { targetNights: number; nightDelta: number } => !!v)
      .sort((a, b) => (Math.abs(a.shiftDays) - Math.abs(b.shiftDays)) || (a.nightDelta - b.nightDelta));

    return candidates[0] ?? null;
  }, [shouldComputeSuggestions, requestedRangeNights, minStay, checkIn, blockedNightsSet]);
                ...
                <div className="space-y-2">
                  ...
                  {selectedPropertyOption ? (
                    <div className="rounded-md border border-slate-200 bg-white px-2 py-1.5 text-xs text-slate-700">
                      ...
                      <p className="mt-1 text-[11px] text-slate-500">Closest available to your requested dates (±1/±2/±3 nights).</p>
                      <button
                        type="button"
                        onClick={() => {
                    ...
                    <div className="rounded-md border border-slate-200 bg-white px-2 py-1.5 text-xs text-slate-700">
                      No close match found for selected property within ±1/±2/±3 nights.
                    </div>
                  )}
                </div>
